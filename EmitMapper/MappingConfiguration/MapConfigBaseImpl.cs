...
namespace EmitMapper.MappingConfiguration
{
    public abstract class MapConfigBaseImpl : IMappingConfigurator
    {
        ...
        public virtual IRootMappingOperation GetRootMappingOperation(Type from, Type to)
        {
            ...
            var mappingOperation = new RootMappingOperation(from, to);

            mappingOperation.TargetConstructor = _customConstructors.GetValue(new[] { to });
            mappingOperation.NullSubstitutor = _nullSubstitutors.GetValue(new[] { to });
            mappingOperation.ValuesPostProcessor = _postProcessors.GetValue(new[] { to });
            mappingOperation.Converter = converter;
            mappingOperation.DestinationFilter = _destinationFilters.GetValue(new[] { to });
            mappingOperation.SourceFilter = _sourceFilters.GetValue(new[] { from });
